...
package com.traffic.monitor.vo;
...
/*
说明：
    1、设备网络SDK开发包【库文件】里的HCNetSDK.dll、HCCore.dll、PlayCtrl.dll、SuperRender.dll、AudioRender.dll、HCNetSDKCom文件夹等文件均要加载到程序里面
    2、HCNetSDKCom文件夹（包含里面的功能组件dll库文件）需要和HCNetSDK.dll、HCCore.dll一起加载，放在同一个目录下，且HCNetSDKCom文件夹名不能修改
    3、编译环境必须匹配，X86的编译环境（JRE、JDK等都是32位的，测试过的jdk有1.8.0版本和1.6.0）需要加载32位的SDK库，X64的编译环境（JRE、JDK等都是64位的，测试的jdk是1.8.0版本）需要加载64位的SDK库
    4、如果还是加载不成功，SDK库所在路径可以添加到Java变量里面，或者拷贝到JRE的bin文件夹下，或者将SDK库拷贝到system32、sysWOW64目录下
 */
...
//    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(lib.getAbsolutePath(), HCNetSDK.class);

    //如果SDK库加载不成功，也可以使用绝对路径
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("C:\\Users\\Lenovo\\Downloads\\CH-HCNetSDKV6.1.9.4_build20220412_win64_20221013184632\\CH-HCNetSDKV6.1.9.4_build20220412_win64\\库文件\\HCNetSDK.dll", HCNetSDK.class);
    /***宏定义***/
    //常量
...
interface PlayCtrl extends Library {
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("C:\\Users\\Lenovo\\Downloads\\CH-HCNetSDKV6.1.9.4_build20220412_win64_20221013184632\\CH-HCNetSDKV6.1.9.4_build20220412_win64\\库文件\\PlayCtrl",
            PlayCtrl.class);
